...
<body>
		...
		<div class="row-fluid">
			...
			<div class="row-fluid">
				<div class="login-box">
					...
					<form class="form-horizontal" action="{{URL('admin-deshboard')}}" method="post">
						{{csrf_field()}}
						<fieldset>
							
							<div class="input-prepend" title="Username">
								<span class="add-on"><i class="halflings-icon user"></i></span>
								<input class="input-large span10" name="admin_email" id="username" type="text" placeholder="Email Address"/>
							</div>
	...
	<!-- start: JavaScript-->
		...
		<script src="{{asset('backend/js/jquery-migrate-1.0.0.min.j')}}s"></script>
		<script src="{{asset('backend/js/jquery-ui-1.10.0.custom.min.js')}}"></script>
		<script src="{{asset('backend/js/modernizr.js')}}"></script>
		<script src="{{asset('backend/js/bootstrap.min.js')}}"></script>
		<script src="{{asset('backend/js/jquery.cookie.j')}}s"></script>
